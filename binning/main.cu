...
__global__ void gpu_normal_kernel(float *in_val, float *in_pos, float *out,
  ...
  if(idx < grid_size){
    ...
    for(int i = 0; i < num_in; i ++){
      in_val2 = in_val[i] * in_val[i];

      dist = in_pos[i] - (float)idx;
      Pval += in_val2 / (dist * dist);
    }
...
//@@ INSERT CODE HERE
  ...
  if(idx < grid_size){
    // int binIdx = (int)((idx / grid_size) * NUM_BINS);
    // int tmp = bin_ptrs[binIdx];
    for(int i = 0; i < NUM_BINS; i ++){
      int t = bin_ptrs[i];
      int t_2 = bin_ptrs[i + 1];
      // in_val_sorted2 = in_val_sorted[t] * in_val_sorted[t];
      dist_s = in_pos_sorted[t] - (float)idx;
      dist_s2 = dist_s * dist_s;
      dist_e = in_pos_sorted[t_2] - (float)idx;
      dist_e2 = dist_e * dist_e;
      if(dist_s2 > cutoff2 && dist_e2 > cutoff2){
        continue;
      }
      // if(dist_s2 <= cutoff2 || dist_e2 <= cutoff2){
        // in_val_sorted2 = in_val_sorted[i] * in_val_sorted[i];
        // Pval += in_val_sorted2 / dist2;
        for(int j = t; j < t_2 ; j ++){
          in_val_sorted2 = in_val_sorted[j]*in_val_sorted[j];
          dist = in_pos_sorted[j]- (float)idx;
          dist2 = dist * dist;
          if(dist2 <= cutoff2){
            Pval += in_val_sorted2 / dist2; 
          }
...
__global__ void histogram(float *in_pos, int *bin_counts, int num_in,
  ...
  if(idx < num_in){
    atomicAdd(&bin_counts[(int)((in_pos[idx] / grid_size) * NUM_BINS)], 1);
  }
  __syncthreads();
...
__global__ void scan(int *bin_counts, int *bin_ptrs) {
  ...
  //@@ INSERT CODE HERE
  // __shared__ int partialSum[ NUM_BINS];

  // int t = threadIdx.x;
  // int start = 2 *blockIdx.x * blockDim.x;
  // partialSum[t] = bin_counts[start + t];
  // partialSum[t + blockDim.x] = bin_counts[start + blockDim.x + t];
  ...
  // if(t == 511) bin_ptrs[1024] = bin_counts[1023] + T[1023];

  
  __shared__ int T[NUM_BINS];
  int i =  threadIdx.x;
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  // if(i < NUM_BINS){
  //   T[threadIdx.x] =  bin_counts[idx];
  // }else T[threadIdx.x] = 0;
  T[2*i] = bin_counts[2*i];
  T[2*i+1] = bin_counts[2*i+1];
  // __syncthreads();
  // int stride = 1;
  ...
  //   __syncthreads();
  //   int id = (threadIdx.x + 1) * stride * 2 - 1;
  //   if((id + stride) < NUM_BINS){
  //     T[id + stride] += T[id]; 
  //   }
  //   stride = stride / 2;
  // }
  int stride = 1; 
  for (int j = NUM_BINS/2; j > 0; j = j / 2){   
		__syncthreads();  
		if (i < j){
        int id = (threadIdx.x + 1) * stride * 2 - 1;
			  T[id] += T[id - stride];  
  ...
  if (i == 0) 
    T[NUM_BINS - 1] = 0;
	for (int j = 1; j < NUM_BINS; j =j* 2){  
    stride = stride / 2; 
		__syncthreads();  
		if (i < j){
      int id = (threadIdx.x + 1) * stride * 2 - 1;
      float tmp = T[id-stride]; 
      T[id - stride] = T[id];
      T[id] += tmp;
    }
  } 
  __syncthreads();
  bin_ptrs[2*i] = T[2*i];
  bin_ptrs[2*i+1] = T[2*i+1];
  if(i==511)  bin_ptrs[1024] = bin_ptrs[1023]+bin_counts[1023]; 


  // if (idx < NUM_BINS && i>0){
  //   bin_ptrs[idx]=T[i-1];
  // }else if(idx < NUM_BINS && idx==0){
  //   bin_ptrs[idx]=0;
  // }
...
__global__ void sort(float *in_val, float *in_pos, float *in_val_sorted,
  ...
  int newidx = 0;
  if(idx < num_in){
    int binIdx = (int)((in_pos[idx] / grid_size) * NUM_BINS);
    // newidx = bin_ptrs[binIdx + 1] - bin_counts[binIdx];
    // atomicSub(&bin_counts[binIdx], 1);
    int tmp = atomicSub(&bin_counts[binIdx], 1);
    newidx = bin_ptrs[binIdx + 1] - tmp;
